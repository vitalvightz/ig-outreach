...
def run_outreach():
    ...
    creds = ServiceAccountCredentials.from_json_keyfile_name(
        '/etc/secrets/your-google-credentials.json', scope)
    ...
    for i, row in enumerate(data):
        if not row.get("Message"):
            ...
            prompt = (
                f"Write a casual Instagram DM to {name}. Include a short human opening based on this: '{notes}'. "
                f"Then naturally transition into this offer: '{core_message}'. Make sure it flows well and sounds real."
            )
